...
class sale_order_line_report(models.TransientModel):
    ...
    def get_don_ban_hang(self,conditions_add=False):
        ...
        for order_line in order_line_ids:
            line_data.append({
                ...
                'doanh_so': order_line.final_price * order_line.product_uom_qty if conditions_add else -(order_line.final_price * order_line.product_uom_qty)
    ...
    def print_excel(self):
        ...
        merge_format = workbook.add_format({'bold': True, 'align': 'center', 'valign': 'vcenter', 'font_size': '16','border': 1})
        header1_bold_color = workbook.add_format(
            {'bold': True, 'font_size': '14', 'align': 'center', 'valign': 'vcenter','border': 1,'num_format': '#,##0'})
        ...
        body_bold_color = workbook.add_format(
            {'bold': False, 'font_size': '12', 'align': 'left', 'valign': 'vcenter','border': 1,'num_format': '#,##0',})
        back_color = 'A2:I2'
        ...
        end_date = datetime.strptime(self.end_date, DEFAULT_SERVER_DATE_FORMAT).strftime("%d/%m/%Y") if self.end_date else datetime.today().strftime("%d/%m/%Y")
        ...
        row = 4
        summary_header = ['Ngày đặt hàng', 'Số đơn hàng', 'Tên khách hàng', 'Mã nội bộ', 'Tên hàng', 'ĐVT','Tổng số lượng bán', 'Đơn giá','Doanh số bán']
