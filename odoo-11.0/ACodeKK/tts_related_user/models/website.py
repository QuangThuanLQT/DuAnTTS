...
class Website(models.Model):
    ...
    website_is_child_user = fields.Boolean(
        compute='_compute_website_child_user')

    def _compute_website_child_user(self):
        for rec in self:
            rec.website_is_child_user = not (
                request.env.user.is_children)
    ...
    def sale_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False):
        ...
        if self.env.user.is_children == True:
            partner = self.env.user.parent_user_id.partner_id
        sale_order_id = request.session.get('sale_order_id')
        order_id = self.env['sale.order'].sudo().search([('id', '=', sale_order_id)])
