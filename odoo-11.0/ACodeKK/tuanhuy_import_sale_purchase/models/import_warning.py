...
class import_warning_order(models.TransientModel):
    ...
    def update_product_to_invoice(self):
        ...
        for acc_move_id in account_move_ids:
            inv_id  = self.env['account.invoice'].search(['|',('number','=',acc_move_id.name),('move_id','=',acc_move_id.id)])
            if not inv_id:
                voucher_id = self.env['account.voucher'].search(['|',('number','=',acc_move_id.name),('move_id','=',acc_move_id.id)])
                if not voucher_id:
                    count += 1
                    print "STT:" + str(count) + " Account Move: " + str(acc_move_id.id) + " - " + acc_move_id.name
        print "---------------" + str(count)
    ...
    def update_invoice_status_for_so_po(self):
        sale_order_ids = self.env['sale.order'].search([('state', '!=', 'draft'),('invoice_status','=','to invoice')])
        for sale_order_id in sale_order_ids:
            for line in sale_order_id.order_line:
            ...
            if order_id.invoice_ids and order_id.amount_total != sum(order_id.invoice_ids.mapped('amount_total')):
                for invoice in order_id.invoice_ids:
                    for line in order_id.invoice_ids.invoice_line_ids:
                        line._compute_price()
                print "--------------------------%s-%s-%s"%(order_id.id,order_id.amount_total,sum(order_id.invoice_ids.mapped('amount_total')))
    ...
    def update_name_so(self):
        picking_ids = self.env['stock.picking'].search([('state', '=', 'assigned')]).filtered(lambda p: 'WH/IN' in p.name)
        count = 0
    ...
    def auto_create_invoice(self):
        sale_ids = self.env['sale.order'].search([]).filtered(lambda record: record.state != 'done' and all([do.state == 'assigned' for do in record.picking_ids]) and not record.invoice_ids)
        count = 1
            ...
            try :
                for picking in order.picking_ids:
                    for line in picking.pack_operation_product_ids:
                        line.qty_done = line.product_qty
                    picking.do_new_transfer()
                popup_obj.create(popup_obj.default_get(popup_obj._fields)).with_context({'active_ids': order.ids}).create_invoices()
                count += 1
            except:
                continue
        ...
        for record in sale_ids:
            if record.invoice_ids:
                for inv in record.invoice_ids:
                    date_order = datetime.strptime(record.date_order,DEFAULT_SERVER_DATETIME_FORMAT).strftime(DEFAULT_SERVER_DATE_FORMAT)
