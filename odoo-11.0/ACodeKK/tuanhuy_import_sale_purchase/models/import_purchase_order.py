                ...
                for row_no in range(sheet.nrows):
                    ...
                    if row_no == 1:
                        ...
                        if line[0]:
                            ...
                            if line[2]:
                                invoice_date_real = datetime.utcfromtimestamp(
                                    (float(line[2]) - 25569) * 86400.0) or False
                            invoice_number_real = line[3] or False
                            partner_id = self.env['res.partner'].search(
                                [('name', '=', line[4]), ('supplier', '=', True)], limit=1)
                            product_id = self.env['product.product'].search([('default_code', '=', line[5])], limit=1)
                            ...
                            if line[8]:
                                ...
                                tax = self.env['account.tax'].search(
                                    [('amount', '=', number_tax), ('type_tax_use', '=', 'purchase')], limit=1).ids
                                if not tax:
                                    tax = self.env['account.tax'].search(
                                        [('amount', '=', 0), ('type_tax_use', '=', 'purchase')], limit=1).ids
                            if tax:
                    ...
                    if row_no > 1:
                        ...
                        if line[0]:
                            if line[1] == so_chung_tu:
                                product_id = self.env['product.product'].search([('default_code', '=', line[5])],
                                                                                limit=1)
                                product_uom_quantity = int(float(line[6]))
                                price_discount = float(line[10])
                                discount = float(line[9]) * 100
