...
class product_category_ihr(models.Model):
    ...
    def default_get(self, fields):
        ...
        res['property_stock_account_input_categ_id'] = self.env['account.account'].search([('code', '=', '3381')],
                                                                                          limit=1).id
        res['property_stock_account_output_categ_id'] = self.env['account.account'].search([('code', '=', '632')],
                                                                                           limit=1).id
        res['property_stock_valuation_account_id'] = self.env['account.account'].search([('code', '=', '1561')],
                                                                                        limit=1).id
        return res

    # @api.model
    # def default_account_input_categ(self):
    #     account_id = self.env['account.account'].search([('code', '=', '3381')], limit=1)
    #     if account_id:
    #         return account_id
    #
    # @api.model
    # def default_account_output_categ(self):
    #     account_id = self.env['account.account'].search([('code', '=', '632')], limit=1)
    #     if account_id:
    #         return account_id
    #
    # @api.model
    # def default_valuation_account(self):
    #     account_id = self.env['account.account'].search([('code', '=', '1561')], limit=1)
    #     if account_id:
    #         return account_id
    #
    # property_stock_account_input_categ_id = fields.Many2one('account.account', default=default_account_input_categ)
    # property_stock_account_output_categ_id = fields.Many2one('account.account', default=default_account_output_categ)
    # property_stock_valuation_account_id = fields.Many2one('account.account', default=default_valuation_account)
