...
class PurchaseRequestLineMakePurchaseRequisition(models.TransientModel):
    ...
    item_ids = fields.One2many('purchase.request.line.make.purchase.requisition.item', 'wiz_id', string='Items')
    purchase_requisition_id = fields.Many2one('purchase.requisition', string='Purchase Requisition', required=False, domain=[('state', '=', 'draft')])
    ...
    def default_get(self, fields):
        res = super(PurchaseRequestLineMakePurchaseRequisition, self).default_get(fields)
    ...
    def make_purchase_requisition(self):
        ...
        for item in self.item_ids:
            ...
            if not requisition:
                preq_data = self._prepare_purchase_requisition(picking_type_id, company_id)
            ...
            if available_pr_lines:
                ...
                po_line_data = self._prepare_purchase_requisition_line(requisition, item)
...
class PurchaseRequestLineMakePurchaseRequisitionItem(models.TransientModel):
    ...
    wiz_id = fields.Many2one('purchase.request.line.make.purchase.requisition', string='Wizard', required=True, ondelete='cascade', readonly=True)
    ...
    product_qty = fields.Float(string='Quantity to Bid', digits_compute=dp.get_precision('Product UoS'))
...
PurchaseRequestLineMakePurchaseRequisitionItem()