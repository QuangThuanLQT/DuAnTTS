...
class PurchaseRequestLine(models.Model):
        ...
        for rec in self:
            ...
            if rec.requisition_lines:
                if any([pr_line.requisition_id.state == 'done' for pr_line in rec.requisition_lines]):
                    temp_req_state = 'done'
                elif all([pr_line.requisition_id.state == 'cancel' for pr_line in rec.requisition_lines]):
                    temp_req_state = 'cancel'
                elif any([pr_line.requisition_id.state == 'in_progress' for pr_line in rec.requisition_lines]):
                    temp_req_state = 'in_progress'
                elif all([pr_line.requisition_id.state in ('draft', 'cancel') for pr_line in rec.requisition_lines]):
    ...
    requisition_qty = fields.Float(compute='_compute_requisition_qty', string='Quantity in a Bid')
    requisition_state = fields.Selection( compute='_compute_requisition_state', string="Bid Status", type='selection', selection=_PURCHASE_REQUISITION_STATE, store=True, default='none')
