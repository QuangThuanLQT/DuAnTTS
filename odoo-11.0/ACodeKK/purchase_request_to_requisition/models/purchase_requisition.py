...
class PurchaseRequisition(models.Model):
    ...
    def _purchase_request_confirm_message(self):
        ...
        for pr in self:
            ...
            for request_id in requests_dict.keys():
                ...
                message = self._purchase_request_confirm_message_content(pr, request, requests_dict[request_id])
    ...
    def _prepare_purchase_order_line(self, requisition, requisition_line, purchase_id, supplier):
        vals = super(PurchaseRequisition, self)._prepare_purchase_order_line(requisition, requisition_line, purchase_id, supplier)
        vals.update({
            'purchase_request_lines': [(4, line.id) for line in requisition_line.purchase_request_lines],
        })
        return vals
...
class PurchaseRequisitionLine(models.Model):
    ...
    has_purchase_request_lines = fields.Boolean(compute="_compute_has_purchase_request_lines", string="Has Purchase Request Lines")
