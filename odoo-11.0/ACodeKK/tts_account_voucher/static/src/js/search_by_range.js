odoo.define('tts_voucher_search_by_range.tree', function (require) {
    ...
    ListView.include({
        ...
        render_buttons: function ($node) {
            ...
            if (self.model == 'account.voucher') {
                self.button_export = $(QWeb.render('voucher_export_button', {}));
                self.button_export.click(function () {
                    var domain = self.dataset.domain;
                    session.get_file({
                        url: '/export_sales_receipts_excel',
                        dataType: 'json',
                        data: {
                ...
                if (self.$buttons[0].getElementsByClassName('o_voucher_buttons_print_excel').length == 0) {
