odoo.define('dashboard_sale_revenue.sale_dashboard', function (require) {
    "use strict";
    ...
    var SaleDashboardView = KanbanView.extend({
        ...
        events: {
            ...
            'click .action_sale_order': 'action_sale_order',
            'click .action_sale_return': 'action_sale_return',
        ...
        render: function () {
            ...
            var self = this;
            var model = new Model('dashboard.sale.revenue').call('get_sale_order_data').then(function (result) {
                self.isFirefox = typeof InstallTrigger !== 'undefined';
                self.sale_data = result[0]
                return self.fetch_data().then(function (result) {
                    var hr_dashboard = QWeb.render('dashboard_sale_revenue.sale_dashboard', {
                        widget: self,
                    });
        ...
        action_edit_line: function (event) {
            var sale_id = event.currentTarget.getAttribute('data-line-id')
            if (sale_id) {
                var self = this;
                event.stopPropagation();
                event.preventDefault();
        ...
        action_confirm: function (event) {
            var sale_id = event.currentTarget.getAttribute('data-line-id')
            if (sale_id) {
                new Model('sale.order').call('action_confirm_order', [parseInt(sale_id)])
                window.location.reload();
            }
        },
        action_cancel: function (event) {
            ...
            if (sale_id) {
                ...
                return this.do_action({
                    type: 'ir.actions.act_window',
                    res_model: "sale.cancel.popup",
                    views: [[false, 'form']],
                    target: 'new',
                    context: {'default_order_id': parseInt(sale_id)},
                });
            }
        },
        action_set_draft: function (event) {
            var sale_id = event.currentTarget.getAttribute('data-line-id')
            if (sale_id) {
                new Model('sale.order').call('action_draft', [parseInt(sale_id)])
                window.location.reload();
            }
        },
        action_new_customer: function (event) {
            var self = this;
            event.stopPropagation();
            event.preventDefault();
            var model = new Model('res.partner').call('get_domain_new_customer').then(function (result) {
        ...
        action_customer: function (event) {
            ...
            event.stopPropagation();
            event.preventDefault();
            var model = new Model('res.partner').call('get_domain_customer').then(function (result) {
                return self.do_action({
                    name: _t("Customers"),
                    type: 'ir.actions.act_window',
                    res_model: "res.partner",
        ...
        action_sale_order: function (event) {
            var self = this;
            event.stopPropagation();
            event.preventDefault();
            var model = new Model('sale.order').call('get_domain_order').then(function (result) {
                return self.do_action({
        ...
        action_sale_return: function (event) {
            var self = this;
            event.stopPropagation();
            event.preventDefault();
            var model = new Model('sale.order').call('get_domain_order_return').then(function (result) {
                return self.do_action({
                    name: _t("Order Return"),
                    type: 'ir.actions.act_window',
                    res_model: "sale.order",
                    views: [[false, 'list'], [false, 'form']],
                    target: 'current',
    ...
    form_widget.WidgetButton.include({
        on_click: function () {
            ...
            if (this.node.attrs.custom === "do_cancel") {
                ...
                var model = new Model("dashboard.sale.revenue");
    ...
    core.view_registry.add('sale_dashboard_view', SaleDashboardView);
