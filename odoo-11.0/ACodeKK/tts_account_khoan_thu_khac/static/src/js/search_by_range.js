odoo.define('tts_account_khoan_thu_khac.tree', function (require) {
    ...
    ListView.include({
        ...
        render_buttons: function ($node) {
            ...
            if (self.model == 'sale.order' && self.ViewManager.action.xml_id == 'tts_account_khoan_thu_khac.khoan_thu_khac_action') {
                self.button_export = $(QWeb.render('export_button', {}));
                self.button_export.click(function () {
                    var domain = self.dataset.domain;
                    session.get_file({
                        url: '/export_overview_excel',
                        dataType: 'json',
                        data: {
                ...
                if (self.$buttons[0].getElementsByClassName('o_buttons_print_excel').length == 0) {
