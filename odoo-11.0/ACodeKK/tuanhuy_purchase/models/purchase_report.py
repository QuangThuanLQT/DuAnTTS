        ...
        self._cr.execute("""
                create view purchase_report as (
                    ...
                    select
                        ...
                        CASE
                          WHEN s.purchase_order_return = true 
                            THEN SUM(l.product_qty / u.factor * u2.factor * -1)
                            ELSE SUM(l.product_qty / u.factor * u2.factor)
                        ...
                        CASE
                          WHEN s.purchase_order_return = true
                            THEN SUM(l.price_subtotal / COALESCE(cr.rate, 1.0) * -1)::decimal(16,2)
                            ELSE SUM(l.price_subtotal / COALESCE(cr.rate, 1.0))::decimal(16,2)
                        END as price_total,
