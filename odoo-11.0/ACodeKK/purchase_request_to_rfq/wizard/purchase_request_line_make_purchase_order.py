...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def _prepare_purchase_order(self, picking_type, location, company_id):
        ...
        data = {
            ...
            'fiscal_position': (supplier.property_account_position_id and supplier.property_account_position_id.id) or False,
    ...
    def _prepare_purchase_order_line(self, po, item):
        ...
        price = pricelist_obj.price_get(item.product_id.id, qty, po.partner_id.id)[pricelist_id]
        vals = {}
        vals.update({
            'order_id': po.id,
            'name': item.name,
            ...
            'prl_id': item.line_id.id or False
        })
    ...
    def _prepare_purchase_request_line(self):
        ...
        prl_lst = []
        purchs_req_line = self.env['purchase.request.line'].search([('id','in',context.get('active_ids',[]))])
        for prl_rec in purchs_req_line or []:
            prl_lst.append((0,0,
                            {
                                'prl_id': prl_rec.id or False,
                                'product_id': prl_rec.product_id.id or False
                            }))
        return prl_lst
    ...
    def make_purchase_order(self):
        ...
        for item in self.item_ids:
            ...
            if not purchase:
                ...
                po_data['purchase_rl_ids'] = self._prepare_purchase_request_line()
