...
from odoo.addons.base.ir.ir_qweb.assetsbundle import AssetsBundle, JavascriptAsset, AssetError
...
class BabelJavascriptAsset(JavascriptAsset):
    ...
    def get_command(self, filename):
        return ['babel', '--presets', '@babel/preset-react', filename]

    def compile(self, source, filename=False):
        command = self.get_command(filename)
        try:
            compiler = Popen(command, stdin=PIPE, stdout=PIPE,
                             stderr=PIPE)
        except Exception:
            raise AssetError("Could not execute command %r" % command[0])
    ...
    def transpile_babel(self):
        ...
        if self.javascripts:
            ...
            if len(need_transpile) > 0:
                ...
                try:
                    compiled = compiler(source, asset._filename)
                    return compiled
...
class QWeb(models.AbstractModel):
    ...
    def _get_asset(self, xmlid, options, css=True, js=True, debug=False, async=False, values=None):
        ...
        asset = AssetsBundleBabel(xmlid, files, remains, env=self.env)
