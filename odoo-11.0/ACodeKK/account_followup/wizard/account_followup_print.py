...
class account_followup_stat_by_partner(models.Model):
    ...
    invoice_partner_id = fields.Many2one('res.partner', 'Invoice Address', compute='_get_invoice_partner_id')
...
class account_followup_sending_results(models.TransientModel):
    ...
    description = fields.Text('Description', readonly=True, default=_get_need_printing)
    needprinting = fields.Boolean('Needs Printing', default=_get_description)
...
class account_followup_print(models.TransientModel):
    ...
    date = fields.Date('Follow-up Sending Date', required=True,
                       help="This field allow you to select a forecast date to plan your follow-ups", default=lambda *a: fields.Date.today(), readonly=True)
    ...
    def process_partners(self, partner_ids, data):
        ...
        for partner in self.env['account_followup.stat.by.partner'].browse(partner_ids):
            if partner.max_followup_id.manual_action:
                partner_obj.browse([int(partner.partner_id.id)]).do_partner_manual_action()
            ...
            if partner.max_followup_id.send_email:
                nbunknownmails += partner_obj.browse([int(partner.partner_id.id)]).do_partner_mail()
                nbmails += 1
            if partner.max_followup_id.send_letter:
                partner_ids_to_print.append(partner.id)
                nbprints += 1
                message = "%s<I> %s </I>%s" % (
                    _("Follow-up letter of "), partner.partner_id.latest_followup_level_id_without_lit.name,
                    _(" will be sent"))
                partner_obj.browse([partner.partner_id.id]).message_post(body=message)
        if nbunknownmails == 0:
            resulttext += str(nbmails) + _(" email(s) sent")
        else:
            resulttext += str(nbmails) + _(" email(s) should have been sent, but ") + str(nbunknownmails) + _(
        ...
        resulttext += "</p>"
        result = {}
        action = partner_obj.browse(partner_ids_to_print).do_partner_print(data)
    ...
    def clear_manual_actions(self, partner_list):
        ...
        partner_list_ids = [partner.partner_id.id for partner in
    ...
    def do_process(self):
        ...
        model_data = mod_obj.search([('model', '=', 'ir.ui.view'),
                                        ('name', '=', 'view_account_followup_sending_results')])
        resource_id = model_data.read(['res_id'])[0]['res_id']
