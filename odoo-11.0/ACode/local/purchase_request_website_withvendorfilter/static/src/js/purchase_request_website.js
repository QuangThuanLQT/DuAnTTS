odoo.define('purchase_request_website.cart_purchase', function (require) {
//     "use strict";
    ...
    var purchase_cart_link = $('ul#top_menu li a[href$="/purchase/cart"]');
    var purchase_cart_link_counter;
    purchase_cart_link.popover({
        trigger: 'manual',
        animation: true,
        html: true,
        title: function () {
        ...
        var self = this;
        clearTimeout(purchase_cart_link_counter);
        purchase_cart_link.not(self).popover('hide');
        purchase_cart_link_counter = setTimeout(function(){
            if($(self).is(':hover') && !$(".mycart-popover:visible").length)
            {
                $.get("/purchase/cart", {'type': 'popover'})
                    .then(function (data) {
...
odoo.define('purchase_request_website.purchase_request_website', function (require) {
    "use strict";
    ...
    $('.oe_website_purchase').each(function () {
        ...
        $(oe_website_purchase).on("change", 'input[name="add_qty"]', function (event) {
            var product_ids = [];
            ...
            if ($("#product_detail").length) {
                ...
                ajax.jsonRpc("/purchase/get_unit_price", 'call', {'product_ids': product_ids,'add_qty': parseInt(qty)})
                .then(function (data) {
                    _.each(product_dom, function (prod) {
        ...
        // change for css
        $(oe_website_purchase).on('mouseup touchend', '.js_publish', function (ev) {
        ...
        $(oe_website_purchase).on("change", ".oe_cart input.js_quantity[data-product-id]", function () {
          var $input = $(this);
          ...
          clickwatch(function(){
            ...
            ajax.jsonRpc("/purchase/cart/update_json", 'call', {
                ...
                $(".js_cart_lines").first().before(data['purchase_request_website.cart_lines']).end().remove();
        ...
        $(oe_website_purchase).on("click", ".oe_cart a.js_add_suggested_products", function () {
            $(this).prev('input').val(1).trigger('change');
        });

        // hack to add and remove from cart with json
        $(oe_website_purchase).on('click', 'a.js_add_cart_json', function (ev) {
            ev.preventDefault();
        ...
        $('.oe_website_purchase .a-submit, #comment .a-submit').off('click').on('click', function (event) {
        ...
        $('form.js_attributes input, form.js_attributes select', oe_website_purchase).on('change', function (event) {
        ...
        $('form.js_add_cart_json label', oe_website_purchase).on('mouseup touchend', function () {
            var $label = $(this);
        ...
        $('.css_attribute_color input', oe_website_purchase).on('change', function () {
        ...
        function update_product_image(event_source, product_id) {
            if ($('#o-carousel-product').length) {
                var $img = $(event_source).closest('tr.js_product, .oe_website_purchase').find('img.js_variant_img');
                $img.attr("src", "/web/image/product.product/" + product_id + "/image");
                $img.parent().attr('data-oe-model', 'product.product').attr('data-oe-id', product_id)
                    .data('oe-model', 'product.product').data('oe-id', product_id);

                var $thumbnail = $(event_source).closest('tr.js_product, .oe_website_purchase').find('img.js_variant_img_small');
                ...
                var $img = $(event_source).closest('tr.js_product, .oe_website_purchase').find('span[data-oe-model^="product."][data-oe-type="image"] img:first, img.product_detail_img');
                $img.attr("src", "/web/image/product.product/" + product_id + "/image");
                $img.parent().attr('data-oe-model', 'product.product').attr('data-oe-id', product_id)
                    .data('oe-model', 'product.product').data('oe-id', product_id);
            }
        ...
        $(oe_website_purchase).on('change', 'input.js_product_change', function () {
            var self = this;
        ...
        $(oe_website_purchase).on('change', 'input.js_variant_change, select.js_variant_change, ul[data-attribute_value_ids]', function (ev) {
            ...
            for (var k in variant_ids) {
                if (_.isEmpty(_.difference(variant_ids[k][1], values))) {
                    ...
                    if (variant_ids[k][3]-variant_ids[k][2]>0.01) {
                        $default_price.closest('.oe_website_purchase').addClass("discount");
        ...
        $('div.js_product', oe_website_purchase).each(function () {
            $('input.js_product_change', this).first().trigger('change');
        });

        $('.js_add_cart_variants', oe_website_purchase).each(function () {
        ...
        $('.oe_cart').on('click', '.js_edit_address', function() {
            $(this).parent('div.one_kanban').find('form.hide').attr('action', '/purchase/address').submit();
        });
        ...
        if ($('.oe_website_purchase .dropdown_sorty_by').length) {
            // this method allow to keep current get param from the action, with new search query
            $('.oe_website_purchase .o_website_sale_search').on('submit', function (event) {
                var $this = $(this);
                if (!event.isDefaultPrevented() && !$this.is(".disabled")) {
