        ...
        if self.pack_operation_product_ids and len(self.pack_operation_product_ids.ids):
            ...
            for pack_line in self.pack_operation_product_ids:
                if pack_line.product_id not in picking_product.keys():
                    ...
                    picking_product.update({pack_line.product_id: picking_product.get(pack_line.product_id) + pack_line.qty_done})
            ...
            for move in self.move_lines:
                if move.product_id not in picking_product.keys():
                    ...
                    picking_product.update({move.product_id: picking_product.get(move.product_id) + move.product_uom_qty})
        ...
        if not missing_product:
            ...
            for product,qty in missing_product.iteritems():
                ...
                if order._name == 'purchase.order':
                    line_data.update({'invoice_line_tax_ids': order_line and order_line[0] and [(6, 0, order_line[0].taxes_id.ids)] or [], 'account_analytic_id': order_line and order_line[0] and order_line[0].account_analytic_id.id or False})
                elif order._name == 'sale.order':
                    line_data.update({'invoice_line_tax_ids': order_line and order_line[0] and [(6, 0, order_line[0].tax_id.ids)] or [], 'account_analytic_id': order.project_id.id or False})
...
class account_invoice(models.Model):
    ...
    def create(self,vals):
        ...
        if self.env.context.get('create_from_picking',False):
            self.env['stock.picking'].browse(self.env.context.get('create_from_picking',False)).write({'invoice_ids': [(4,res.id)]})
        return res