...
class brand_name(models.Model):
    ...
    def action_merge_brand_name(self):
        ...
        if len(brand_name_ids) > 1:
            ...
            action['context'] = {'brand_name_id':brand_name_ids[0].id,'brand_name_ids':(brand_name_ids - brand_name_ids[0]).ids}
            return action
            ...
            for brand_field_id in brand_field_ids:
                ...
                if model == 'product.template':
                    for brand_relate_id in brand_relate_ids:
                        ...
                        sale_order_ids = self.env['sale.order.line'].search([('product_id','=',brand_relate_id.product_variant_id.id)])
                        for line in sale_order_ids:
                            self._cr.execute("""UPDATE sale_order_line SET brand_name='%s' WHERE id=%s""" % (self.brand_name_id.name,line.id))
                        purchase_order_ids = self.env['purchase.order.line'].search([('product_id', '=', brand_relate_id.product_variant_id.id)])
                        for line in purchase_order_ids:
                            self._cr.execute("""UPDATE purchase_order_line SET brand_name='%s' WHERE id=%s""" % (self.brand_name_id.name, line.id))
                else:
                    for brand_relate_id in brand_relate_ids:
                        try:
                            self._cr.execute("""UPDATE %s SET %s=%s WHERE id=%s""" % (
                                model.replace(".", "_"), brand_field_id.name, self.brand_name_id.id, brand_relate_id.id))
                        except:
                            pass
...
class source_name(models.Model):
    ...
    def update_source_name(self):
        ...
        for invoice_id in invoice_ids:
            move_line_ids = invoice_id.move_id.mapped('line_ids').filtered(lambda mvl:mvl.account_id.code in ['131','331'])
            for move_line in move_line_ids:
                if not move_line.amount_residual:
                    print "INV: (%s,%s) - MVL: %s"%(invoice_id.id,invoice_id.number,move_line.id)
