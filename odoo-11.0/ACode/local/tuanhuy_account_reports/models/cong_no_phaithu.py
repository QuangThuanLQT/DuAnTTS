...
class CongNoPhaithu(models.Model):
    ...
    _name        = 'cong.no.phaithu'
    _description = 'Cong No 131 Chi Tiet'
    _auto        = False
    ...
    ref_account_payment_gbn = fields.Many2one('account.payment.gbn', string="Số chứng từ", compute="_get_ref_account_payment_gbn")
    ...
    def _get_ref_sale_order(self):
        for record in self:
            if record.account_du_id.code in ['5111','33311','5213'] and record.ref:
                sale_id = self.env['sale.order'].search([('name','=',record.ref)])
                if sale_id:
    ...
    def _get_ref_account_voucher(self):
        for record in self:
            if record.account_du_id.code in ['1111','1121'] and record.ref:
                voucher_id = self.env['account.voucher'].search([('number_voucher','=',record.ref),('voucher_type','=','sale')])
                if voucher_id:
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        res = super(CongNoPhaithu, self.with_context(context)).search_read(domain=domain, fields=fields,offset=offset, limit=limit, order=order)
        return res
    ...
    def _compute_after(self):
        ...
        if args and any(['partner_id' in a for a in args]):
            partner = [x for x in args if ('partner_id' in x and 'in' in x)]
            partner_id = list(partner[0][2]) if partner else False
        for record in self:
            no_before, co_before = record._get_debit_credit_before_to_131(partner_id,date)
    ...
    @api.model
    def _get_debit_credit_before_to_131(self, partner_id,date):
