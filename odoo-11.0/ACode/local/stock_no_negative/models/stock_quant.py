...
class StockQuant(models.Model):
    ...
    @api.multi
    @api.constrains('product_id', 'qty', 'location_id')
    def check_negative_qty(self):
        p = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        if (config.get('test_enable') and not self.env.context.get('test_stock_no_negative')):
            return
        for quant in self:
            if (float_compare(quant.qty, 0, precision_digits=p) == -1
                ...
                and not quant.location_id.allow_negative_stock):
                msg_add = ''
                if quant.lot_id:
