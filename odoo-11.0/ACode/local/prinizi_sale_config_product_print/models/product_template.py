...
class product_template_ihr(models.Model):
    ...
    def create(self, val):
        ...
        res.create_config_product_print()
        return res
    ...
    def write(self, val):
        ...
        if 'attribute_line_ids' in val:
            self.create_config_product_print()
        return res
    ...
    def unlink(self):
        ...
        for rec in self:
            product_print_ids = self.env['config.product.print'].search([('product_id', '=',rec.id)])
            product_print_ids.unlink()
        return res

    @api.multi
    def create_config_product_print(self):
        for rec in self:
            attribute_line_ids = rec.attribute_line_ids.filtered(lambda l: l.attribute_id.name in ('Màu','màu','Mau','mau'))
            product_print_ids = self.env['config.product.print'].search([('product_id', '=', rec.id)]).mapped('attribute_value_id')
            for attribute_line_id in attribute_line_ids:
                for value_id in attribute_line_id.value_ids:
                    if value_id not in product_print_ids:
                        self.env['config.product.print'].create({
                            'product_id': rec.id,
