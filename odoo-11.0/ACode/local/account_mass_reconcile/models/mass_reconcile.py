...
class MassReconcileOptions(models.AbstractModel):
    ...
    income_exchange_account_id = fields.Many2one('account.account', string='Gain Exchange Rate Account')
    expense_exchange_account_id = fields.Many2one('account.account', string='Loss Exchange Rate Account')
...
class AccountMassReconcileMethod(models.Model):
    ...
    def _get_all_rec_method(self):
        if self.env.user.id == 1:
            return [
                ('mass.reconcile.simple.name', 'Số Tiền và Tên'),
                ('mass.reconcile.simple.partner', 'Số Tiền và Đối Tác'),
                ('mass.reconcile.simple.partner.ref', 'Đối tác và Tham Chiếu'),
                ('mass.reconcile.simple.reference', 'Số Tiền và Tham Chiếu'),
                ('mass.reconcile.advanced.ref', 'Số Tiền và Tham Chiếu - Mở rộng'),
                ('mass.reconcile.advanced.partner', 'Đối Tác - Mở rộng'),
            ]
        else:
            return [
                ('mass.reconcile.simple.partner.ref', 'Đối tác và Tham Chiếu'),
    ...
    exclude_partner_ids = fields.Many2many(
        'res.partner', 'reconcile_method_exclude_partner_rel', 'method_id', 'partner_id',
        string='Exclude on partners',
    )
    partner_ids = fields.Many2many(
        'res.partner', 'reconcile_method_partner_rel', 'method_id', 'partner_id',
        string='Partners',
    )
...
class AccountMassReconcile(models.Model):
    ...
    def _prepare_run_transient(self, rec_method):
        start_date = False
        if rec_method.task_id.start_date:
            if isinstance(rec_method.task_id.start_date, basestring):
                start_date = '%s 00:00:00' %(rec_method.task_id.start_date,)
            else:
                start_date = rec_method.task_id.start_date.strftime('%Y-%m-%d 00:00:00')

        end_date = False
        if rec_method.task_id.end_date:
            if isinstance(rec_method.task_id.end_date, basestring):
                end_date = '%s 23:59:59' %(rec_method.task_id.end_date,)
            else:
                end_date = rec_method.task_id.end_date.strftime('%Y-%m-%d 23:59:59')
        ...
        return_data = {
            ...
            'end_date': end_date,
            'partner_ids': [(4, r.id) for r in rec_method.partner_ids],
        }
        if rec_method.exclude_partner_ids:
            if len(rec_method.exclude_partner_ids) == 1:
                return_data.update({
                    'filter': [('partner_id', '!=', rec_method.exclude_partner_ids.id)],
                })
            elif len(rec_method.exclude_partner_ids) > 1:
                return_data.update({
                    'filter': [('partner_id', 'NOT IN', rec_method.exclude_partner_ids.ids)],
                })
        return return_data
        ...
        for rec in self:
            ...
            try:
                ...
                for method in rec.reconcile_method:
                    ...
                    if method.name == 'mass.reconcile.simple.partner.ref':
                        method.write_off = 1000000000000
                ...
                message = "There was an error during reconciliation : %s" % e.name
                ...
                message = "There was an error during reconciliation : %s" % e.message
                rec.message_post(body=message)
                self.env['mass.reconcile.history'].create({
                    'mass_reconcile_id' : rec.id,
                    'date'              : fields.Datetime.now(),
